...
export default function RedeemQrCode() {
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    var redeemUrl =
      baseurl + "/api/farm-field-tickets/" + output.id + "/redeem";
    ...
    axios
      .all([postTicketRequest, getFarmFieldNameRequest])
      .then(
        axios.spread((...responses) => {
          ...
          if (postTicketResponse.status === 200) {
            alert(
              " Field: " +
                getFarmFieldNameResponse.data.name +
                "\n" +
                " Small Buckets: " +
                output.smallBucketQty +
                "\n" +
                " Medium Buckets:" +
                output.mediumBucketQty +
                "\n " +
                "Large Buckets: " +
                output.largeBucketQty
            );
  ...
  return (
    ...
    >
      ...
      {scanned && (
        <Button
          title={"Tap to Scan Another Ticket"}
